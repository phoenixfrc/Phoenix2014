	...
	switch (m_shooterState){
		...
		case braking:
			...
			if (isBraked){
				brakeMotor.Set(0.0);
				m_shooterState = unwinding;
				unwindDelayCounter = 2000; //this is 10 seconds at 200 iters per second
			}
			break;
		case unwinding:
...
void Shooter::DisplayDebugInfo(DriverStationLCD::Line line, DriverStationLCD * lcd){
	...
	lcd->PrintfLine(line, "Ss=%c%c%c", //prints the button values to LCD display
					isWound ? '1':'0',
					isBraked ? '1':'0',
					isUnbraked ? '1':'0'
					);
